...
matlabbatch{1}.spm.spatial.smooth.prefix = ['smo' num2str(Para.Smooth.FWHM(1)) num2str(Para.Smooth.FWHM(2)) num2str(Para.Smooth.FWHM(3)) '_'];
...
for i = 1:Num_subs
    ...
    cd ([Dir_data{1}{i}])
    imgs = spm_select('ExtList',pwd, ['^' File_filter  '.*\.nii$'],inf);
    if isempty(imgs)
        imgs = spm_select('ExtList',pwd, ['^' File_filter  '.*\.img$'],inf);
    end
    
    Num_imgs = size(imgs,1);
    datacell = cell(Num_imgs,1);
    for j = 1:Num_imgs
        datacell{j,1} = [pwd filesep deblank(imgs(j,:))];
    end
